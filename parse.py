...
def reformat_query(query: str) -> str:
    query = query.strip().replace(";", "").replace("\t", "") #remove semicolons and tabs
    query = ' '.join([t.value for t in tokenize(query) if t.ttype != sqlparse.tokens.Whitespace]) #tonize query, remove whitespace tokens
    t_stars = ["t1.*", "t2.*", "t3.*", "T1.*", "T2.*", "T3.*"] 
    for ts in t_stars:
        query = query.replace(ts, "*") # replace " t<digit>.* " by " * "
    return query
...
def replace_values(sql: str) -> Tuple[List[str], Set[str]]:
    ...
    sql = re.sub(r"(T\d+\.)\s", r"\1", sql) #replace " T<digit>.<name> " by " <digit> "
    query_toks_no_value, values = strip_query(sql)
    return query_toks_no_value, set(values) #return the set of all available values
...
def get_all_preds_for_execution(gold: str, pred: str, s_values) -> Tuple[int, Iterator[str]]:
    _, gold_values = extract_query_values(gold)

    #To use both s_values and gold_values for neighbor generation
    #append min, max values to gold_values
    for val in s_values:
        gold_values.add(val)

    # #Uncomment below to use only s_values for neighbor generation    
    # gold_values = set(s_values)
