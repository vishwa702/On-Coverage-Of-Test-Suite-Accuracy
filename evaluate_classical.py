...
if NUM_PROCESSES == 0:
    NUM_PROCESSES = 1
    print('NUM_PROCESSES == 0')
else:
        print('NUM_PROCESSES == ', cpu_count())
...
def main(preds: List[str], gold_file: str = "classical_test.pkl", verbose: bool = True,
    ...
    with Pool(num_processes) as pool:
        ...
        #result = list(tqdm.tqdm(pool.starmap(judge, zip(gold_dicts, preds, repeat(use_cache, len(preds)))), total=len(gold_dicts)))
        #result = list(tqdm.tqdm(pool.imap(judge, zip(gold_dicts, preds, repeat(use_cache, len(preds)))), total=len(gold_dicts)))
